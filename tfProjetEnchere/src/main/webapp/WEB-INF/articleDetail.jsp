				...
				<div class="row">
							...
							<div class="card-body pt-0 px-0">
								...
								<div class="d-flex flex-row justify-content-between px-3 pb-2">
									<div class="d-flex flex-column">
										<span class="text-muted">${article.nomArticle}</span><small
											class="text-muted">${article.description}</small>
									</div>
								</div>
							</div>
						</div>
					...
					<div class="col-12 col-sm-12 col-md-6">
						<div class="card w-responsive">
							<button type="button" class="btn btn-secondary shadow-1" disabled>
								<h4 class="text-center pt-2">PRIX DE DÉPART :
									${article.misAPrix} POINTS</h4>
							</button>
							<div class="card-body pt-0 px-0">
								<div
								...
								<div class="d-flex flex-row justify-content-between px-3 pb-2">
									<div class="d-flex">
										<span class="text-muted pseudoUtilisateur">ADRESSE DE
											RETRAIT :<br>
										<strong> ${retrait.rue} <br>${retrait.codePostal}
												${retrait.ville}
										</strong>
										</span>
									</div>
									<button type="button"
										class="btn btn-success btnNewPrice shadow-1 mb-2" disabled>
										<c:if test="${prixVente==null}">
											<h6 class="text-center pt-1">
												DERNIER PRIX :<br> ${article.misAPrix} POINTS
											</h6>
										</c:if>
										<c:if test="${prixVente!=null}">
											<h6 class="text-center pt-1">
												DERNIER PRIX :<br> ${prixVente} POINTS
											</h6>
										</c:if>
								...
								<div class="d-flex flex-row justify-content-center p-3 mid">
									<c:if test="${!isAllowed}">
										<small class="text-center">*enregistrez-vous pour
											enchèrir</small>
									</c:if>
									<c:if test="${isAllowed}">
										<form
											action="${pageContext.request.contextPath}/articleDetail"
												...
												<div class="input-group">
													...
													<c:if test="${prixVente==null}">
														<input type="number" class="form-control"
															id="inlineFormInputGroupUsername" name="creditEnchere"
															placeholder="${article.misAPrix}"
															min="${article.misAPrix}" step="10" />
													</c:if>
													<c:if test="${prixVente!=null}">
														<input type="number" class="form-control"
															id="inlineFormInputGroupUsername" name="creditEnchere"
															placeholder="${prixVente}" min="${prixVente}" step="10" />
													</c:if>
												</div>
											</div>
