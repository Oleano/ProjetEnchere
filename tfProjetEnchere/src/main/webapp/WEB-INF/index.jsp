...
<body>
	...
	<header>
		<!-- Navbar -->
		<nav
			class="navbar navbar-expand-lg navbar-light bg-transparent shadow-0">
			<!-- Container wrapper -->
			<div class="container">
				<!-- Navbar brand -->
				<a class="navbar-brand me-2" href="<%=request.getContextPath()+"/EnchereServlet"%>">
					<img src="img/logo-navbar.png" height="35" alt="ENI LOGO"
							...
							<c:if test="${isAllowed}">
								<li class="nav-item"><a class="nav-link"
									href="${pageContext.request.contextPath}/VenteArticleServlet">Vendre
										un Article</a></li>
								<li class="nav-item"><a class="nav-link"
									href="${pageContext.request.contextPath}/ProfilUtilisateurServlet">Mon
										Profil</a></li>
						</ul>
					</div>
					<div class="d-flex">
						...
						<c:if test="${!isAllowed}">
							<a href="${pageContext.request.contextPath}/connexion"><button
									type="button" class="btn btn-secondary me-3 mb-2 shadow-2">CONNEXION</button></a>
							<a href="${pageContext.request.contextPath}/connexion"><button
									type="button" class="btn btn-primary me-3 shadow-2">CREER
									UN COMPTE</button></a>
						</c:if>
						<c:if test="${isAllowed}">
							<a class="nav-link"
	...
	<main class="container">
		...
		<div
			class="container containerFilter mt-5 col-12 col-sm-12 col-md-8 col-lg-8 col-xl-6">
			<section id="filter"
				class="mid2 row w-100 mx-auto border border rounded p-4 shadow-2">
				<span>Filtres :</span>
				<div class="form-outline my-2">
					<i class="fas fa-search trailing"></i> <input type="text"
						name="articleIn" id="searchBarInput" class="form-control"
						placeholder="Le nom de l'article contient" onkeyup="searchBar()" />
				</div>
				...
				<form method="post"
						...
						<div class="row">
							<div class="col-12 col-sm-12 col-md-12 col-lg-6 col-xl-6 mb-3">
								<input type="radio" name="filterRadio" id="achats"
									onchange="check()" checked> <label for="achats">Achats</label>
								<div>
									<div>
							...
							<div class="col-12 col-sm-12 col-md-12 col-lg-6 col-xl-6 mb-3">
								<input type="radio" name="filterRadio" id="ventes"
									${disableRadio} onchange="check()"> <label for="ventes">Mes
									ventes</label>
								<div>
									<div>
										<input type="checkbox" name="ventesEnCours" id="ventesEnCours"
											disabled> <label for="ventesEnCours">Mes
											ventes en cours</label>
									</div>
									<div>
										<input type="checkbox" name="ventesNonCommencees"
											id="ventesNonCommencees" disabled> <label
											for="ventesNonCommencees">Ventes non débutées</label>
									</div>
									<div>
					...
					<button type="button"
						class="btn btn-light w-100 text-center mt-1 shadow-2">
						<i class="fas fa-search trailing"></i> Rechercher
					</button>
				</form>
			</section>
		</div>
			...
			<div class="container-fluid d-flex justify-content-center">
				<div class="row mt-4">
					...
					<c:forEach var="element" items="${article}">
						<div
							class="articleSearch col-xl-4 col-lg-4 col-md-12 col-sm-12 col-12 mt-3">
							<div class="card">
								...
								<div class="card-body pt-0 px-0">
									...
									<div class="d-flex flex-row justify-content-between px-3 pb-2">
										<div class="d-flex flex-column">
											<span class="text-muted">${element.nomArticle}</span><small
												class="text-muted">${element.description}</small>
										</div>
									</div>
									<div class="d-flex flex-row justify-content-between p-3 mid">
										<div class="d-flex flex-column">
											<small class="text-muted mb-1">VENDEUR</small>
											<div class="d-flex flex-row">
												<div class="d-flex flex-column ml-1">
													<h6 class="pseudoUtilisateur ml-1">${element.vendeur.pseudo}</h6>
												</div>
											</div>
										</div>
										<div class="d-flex flex-column">
											<small class="text-muted mb-1">FIN DE L'ENCHERE</small>
											<div class="d-flex flex-row">
												<h6 class="ml-1">${element.finEnchere}</h6>
											</div>
										</div>
										<div class="d-flex flex-column">
											<small class="text-muted mb-1">DERNIER PRIX</small>
											<div class="d-flex flex-row">
												<h6 class="ml-1 dernierPrix">${element.prixVente}</h6>
											</div>
										</div>
									</div>
									<div class="mx-3 mt-2 mb-2">
										<a
											href="<%=request.getContextPath()%>/articleDetail?action=getDetail&noArticle=${element.noArticle}"><button
												type="button" class="btn btn-outline-danger btn-block">
												<small>DETAIL DE L'ARTICLE</small>
											</button></a>
									</div>
									<c:if test="${!isAllowed}">
										<small class="d-flex justify-content-center text-muted">*enregistrez-vous
											pour enchérir</small>
									</c:if>
								</div>
							</div>
		...
		<section class="">
			<!-- Footer -->
			<footer class="text-center text-white rounded-2 mb-2"
				style="background-color: #A1A0A0FF;">
				<!-- Grid container -->
					...
					<c:if test="${!isAllowed}">
						<section class="">
							<p class="d-flex justify-content-center align-items-center">
								<span class="me-3">Enregistrez-vous Gratuitement</span> <a
									class="h2 text-primary mx-3"
									href="${pageContext.request.contextPath}/connexion"><button
										type="button" class="btn btn-outline-light btn-rounded">
										CREER MON COMPTE</button></a>
							</p>
						</section>
					</c:if>
					<!-- Section: CTA -->
				...
				<div class="text-center p-3"
					...
					© 2022 Copyright : Virginie MERLINO / Kenza BELHADI / Thomas
					FANOUILLERE</a>
				</div>
				<!-- Copyright -->
			</footer>
